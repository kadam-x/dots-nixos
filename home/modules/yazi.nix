{ ... }:
{
  ...
  xdg.configFile = {
    ...
    "yazi/yazi.toml".text = ''
      ...
      # Extract zip/archive into a same-named directory, then enter it
      extract = [
        { run = 'unar -o "$(dirname "$1")" "$1"', block = true, for = "unix" },
      ]
    ...
    # ── theme.toml ───────────────────────────────────────────────────────────
    # Points yazi at the base16.yazi flavor for both light and dark.
    # The flavor itself must live at ~/.config/yazi/flavors/base16.yazi/
    # Run once: yazi pack -a matt-dong-123/base16  (or nix-shell -p yazi and run it)
    ...
    # ── package.toml ─────────────────────────────────────────────────────────
    # Declares the flavor dependency so `yazi pack` knows what to fetch/lock.
    "yazi/package.toml".text = ''
      [plugin]
      deps = []

      [[flavor.deps]]
      use  = "matt-dong-123/base16"
      rev  = "b02b7a8"
      hash = "fdb3a7d76f51c0a46bde55322ede9171"
    '';
    ...
    "yazi/keymap.toml".text = ''
      ...
      keymap = [
        ...
        { on = "<C-c>",     run = "close",               desc = "Close the current tab, or quit if it is last" },
        ...
        { on = [ "g", "s" ],       run = "yazi sftp://my-server", desc = "Go to my-server" },
